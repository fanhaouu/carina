...
func NewPodReconciler(
	...
	nodeName string,
	stopChan <-chan struct{},
	partition partition.LocalPartition) *PodReconciler {
...
func (r *PodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.Infof("Try to congfig pod's pv blkio, pod namespace: %s, pod name: %s", pod.Namespace, pod.Name)
...
func (r *PodReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	go func(t *time.Ticker) {
		...
		for {
			select {
			case <-t.C:
				_ = r.handleAllPodCGroupConfig(ctx)
			case <-r.stopChan:
				log.Info("Shutting down pod controller")
				return
			}
		}
	}(ticker)
...
func (r *PodReconciler) handleSinglePodCGroupConfig(ctx context.Context, pod *corev1.Pod) error {
	...
	return iolimit.SetIOLimit(r.getPodBlkIO(ctx, pod))
}

func (r *PodReconciler) handleAllPodCGroupConfig(ctx context.Context) error {
	log.Info("Handle all pod cgroup blkio")

	podList := &corev1.PodList{}
	err := r.Client.List(ctx, podList, client.MatchingFields{"combinedIndex": r.nodeName})
	if err != nil {
		return err
	}
	for _, pod := range podList.Items {
		go r.handleSinglePodCGroupConfig(ctx, &pod)
	}
	return nil
}
...
func (p podFilter) filter(pod *corev1.Pod) bool {
	...
	var ioLimit bool
	for _, v := range iolimit.GetSupportedIOThrottles() {
		if _, ok := pod.Annotations[fmt.Sprintf("%s/%s", KubernetesCustomized, v)]; ok {
			ioLimit = true
			break
		}
	}
